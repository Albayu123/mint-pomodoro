    ...
    <div className="max-w-4xl mx-auto p-6 md:p-12 h-full flex flex-col">
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          ...
          className="px-4 py-2 bg-green-500 text-black font-bold rounded-xl hover:bg-green-400 transition-colors flex items-center gap-2"
        >
      ...
      {isAdding && (
        <form onSubmit={handleAddTask} className="mb-8 bg-[#1a1a1a] p-6 rounded-2xl border border-green-500/20 animate-fade-in shadow-xl shadow-green-900/10">
            ...
            <div className="flex items-end gap-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-6 py-3 bg-green-500 text-black font-bold rounded-xl hover:bg-green-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
                  Save Task
                </button>
              </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
        {tasks.length === 0 ? (
          <div className="text-center py-20 opacity-50">
            ...
            <p className="text-xl font-medium text-gray-400">No tasks yet</p>
...
const TaskItem: React.FC<{ task: Task; onToggle: () => void; onDelete: () => void }> = ({ task, onToggle, onDelete }) => (
  <div className={`group bg-[#1a1a1a]/50 hover:bg-[#1a1a1a] border border-gray-800/50 hover:border-gray-700 p-4 rounded-xl transition-all flex items-center gap-4 ${task.completed ? 'opacity-50' : ''}`}>
    <button
      onClick={onToggle}
      className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${task.completed ? 'bg-green-500 border-green-500' : 'border-gray-600 hover:border-green-500'
        }`}
    >
    ...
    <button
      onClick={onDelete}
      className="text-gray-600 hover:text-red-400 p-2 rounded-lg hover:bg-red-400/10 opacity-0 group-hover:opacity-100 transition-all"
    >
      <Icons.Trash />
    </button>
  </div>
